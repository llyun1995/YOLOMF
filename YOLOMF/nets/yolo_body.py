...
from .mobilenet_v3_FusedIR import mobilenetv3_FusedIR
...
class MobilenetV3_FusedIR(nn.Module):
    def __init__(self, pretrained = False):
        super(MobilenetV3_FusedIR, self).__init__()
        self.model = mobilenetv3_FusedIR(pretrained=pretrained)
...
def conv2d_1(filter_in, filter_out, kernel_size, groups=1, stride=1):
    pad = (kernel_size - 1) // 2 if kernel_size else 0
    return nn.Sequential(OrderedDict([
        ("conv", nn.Conv2d(filter_in, filter_out, kernel_size=kernel_size, stride=stride, padding=pad, groups=groups, bias=False)),
        ("bn", nn.BatchNorm2d(filter_out)),
        ("h_swish", h_swish(inplace=True)),
    ]))
...
def conv_dw_1(filter_in, filter_out, stride = 1):
    return nn.Sequential(
        nn.Conv2d(filter_in, filter_in, 3, stride, 1, groups=filter_in, bias=False),
        nn.BatchNorm2d(filter_in),
...
def make_three_conv(filters_list, in_filters):
    m = nn.Sequential(
        conv2d_1(in_filters, filters_list[0], 1),
        conv_dw_1(filters_list[0], filters_list[1]),
        conv2d_1(filters_list[1], filters_list[0], 1),
    )
    return m
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, backbone="mobilenetv3_FusedIR", pretrained=False):
        ...
        if backbone == "mobilenetv1":
        ...
        elif backbone == "mobilenetv3_FusedIR":
            # 52,52,40；26,26,112；13,13,160
            self.backbone = MobilenetV3_FusedIR(pretrained=pretrained)
            in_filters = [40, 112, 160]
            ...
            raise ValueError('Unsupported backbone - `{}`, Use mobilenetv1, mobilenetv2, mobilenetv3, ghostnet, vgg, densenet121, densenet169, densenet201, resnet50, mobilenet_v3_FusedIR.'.format(backbone))

        # SPP+PANet+Head
        self.conv1           = make_three_conv([512, 1024], in_filters[2])   # make_three_conv(filters_list, in_filters)
        self.SPP             = SpatialPyramidPooling()
        self.conv2           = make_three_conv([512, 1024], 2048)

        self.upsample1       = Upsample(512, 256)
        self.conv_for_P4     = conv2d(in_filters[1], 256,1)          # k1,s1,c256
        self.make_five_conv1 = make_five_conv([256, 512], 512)

        self.upsample2       = Upsample(256, 128)
        self.conv_for_P3     = conv2d(in_filters[0], 128,1)          # k1,s1,c128
        self.make_five_conv2 = make_five_conv([128, 256], 256)

        # 3*(5+num_classes) = 3*(5+20) = 3*(4+1+20)=75
        self.yolo_head3      = yolo_head([256, len(anchors_mask[0]) * (5 + num_classes)], 128)

        self.downsample1    = conv_dw(128, 256, stride = 2)         # k3,s2,c256; 深度可分离卷积替换普通3×3卷积，改造瓶颈结构
        self.make_five_conv3 = make_five_conv([256, 512], 512)

        # 3*(5+num_classes) = 3*(5+20) = 3*(4+1+20)=75
        self.yolo_head2      = yolo_head([512, len(anchors_mask[1]) * (5 + num_classes)], 256)

        self.downsample2    = conv_dw(256, 512, stride = 2)         # k3,s2,c256; 深度可分离卷积替换普通3×3卷积，改造瓶颈结构
